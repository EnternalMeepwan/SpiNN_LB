...
// ! the index of the lattice in the the network
...
void initFi(float *fi, float rho)
{
    ...
    for (int k = 0; k < Q; k++)
    {
                ...
                (1.0 + A * (ex[k] * u_x + ey[k] * u_y) //  A c_ia u_a =  A (c_ix u_x + c_iy u_y)
...
static bool initialize(uint32_t *timer_period)
{
    ...
    position_data_t *position_data_sdram =
        data_specification_get_region(POSITION, data);
    x_pos = position_data_sdram->x_position;
    y_pos = position_data_sdram->y_position;
    log_info("I'm in position ( %d, %d)", x_pos, y_pos);

    neighbour_keys_t *neighbour_data_sdram =
        data_specification_get_region(NEIGHBOUR_KEYS, data);
